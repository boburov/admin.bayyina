...
// Data
import { salariesKeys, formatMonthLabel } from "@/features/salaries/data/salaries.data";
...
const SalaryDetailModal = () => (
  <ResponsiveModal name="salaryDetail" title="Oylik tafsiloti" className="max-w-lg">
    <Content />
  </ResponsiveModal>
);
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">{formatMonthLabel(salary.month)}</p>
        </div>
        ...
        <div>
          ...
          <div className="border border-gray-200 divide-y divide-gray-100 rounded-md">
            {salary.groups.map((g, i) => (
              <div key={i} className="flex items-start justify-between px-3 py-2.5 gap-2">
                <div>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-md px-4 py-3 space-y-1.5 text-sm">
        <Row label="Hisoblangan" value={`${salary.totalAmount?.toLocaleString()} so'm`} />
        {salary.bonus > 0 && (
          <Row label="Bonus" value={`+${salary.bonus?.toLocaleString()} so'm`} green />
        )}
        {salary.deduction > 0 && (
          <Row label="Jarima" value={`-${salary.deduction?.toLocaleString()} so'm`} red />
        )}
        <div className="border-t border-gray-200 pt-1.5">
          <Row label="Sof to'lov" value={`${salary.netAmount?.toLocaleString()} so'm`} bold />
        </div>
...
const Row = ({ label, value, bold, green, red }) => (
  <div className={`flex justify-between ${bold ? "font-semibold text-gray-900" : "text-gray-600"}`}>
    <span>{label}</span>
    <span className={green ? "text-green-600" : red ? "text-red-500" : ""}>{value}</span>
  </div>
);
